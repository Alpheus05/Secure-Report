...
async function run() {
  ...
  assert.equal(store.getUnacknowledgedResidentReports().length, 1);
  ...
  assert.equal(store.getReport(different.report.id).adminReview.acknowledged, true);
  assert.equal(store.getUnacknowledgedResidentReports().some((report) => report.id === different.report.id), false);
  ...
  assert.equal(store.getReport(duplicate.report.id).adminReview.acknowledged, true);
  assert.equal(store.getReport(first.report.id).evidence[0].imageRef, "PHOTO-RESIDENT-001");
  assert.ok(store.getIncident(first.incident.id).linkedReportIds.includes(first.report.id));

  const lateLinked = store.createReport({
    description: "A later resident report for the already reviewed outage.",
    outageType: "Power outage",
    issueType: "Household issue",
    latitude: -25.7501,
    longitude: 28.1901,
  });
  assert.equal(lateLinked.incident.id, first.incident.id);
  assert.equal(lateLinked.report.adminReview.acknowledged, false);
  assert.equal(store.getUnacknowledgedResidentReports().some((report) => report.id === lateLinked.report.id), true);
  assert.equal(store.getUnacknowledgedResidentReports().some((report) => report.id === first.report.id), false);
  assert.equal(store.getUnacknowledgedResidentReports().some((report) => report.id === duplicate.report.id), false);
  ...
  assert.equal(store.getReport(first.report.id).adminReview.acknowledged, true);
  assert.equal(store.getReport(duplicate.report.id).adminReview.acknowledged, true);
  assert.equal(store.getUnacknowledgedResidentReports().some((report) => report.id === first.report.id), false);
  assert.equal(store.getUnacknowledgedResidentReports().some((report) => report.id === lateLinked.report.id), true);
  ...
  approvalStore.subscribeToStore(function (change) { approvalDomains.push(change.domain); });
  const approvalFirst = approvalStore.createReport({
    description: "First linked resident report.",
    outageType: "Cable fault",
    latitude: -25.7547,
    longitude: 28.1468,
    evidence: [{ id: "EVID-A", source: "resident", imageRef: "PHOTO-A", capturedAt: "2026-09-21T09:00:00.000Z" }],
  });
  const approvalSecond = approvalStore.createReport({
    description: "Second linked resident report.",
    outageType: "Cable fault",
    latitude: -25.7546,
    longitude: 28.1469,
  });
  assert.equal(approvalSecond.incident.id, approvalFirst.incident.id);
  assert.equal(approvalStore.getUnacknowledgedResidentReports().length, 2);
  const approvalRecommendation = approvalFirst.incident.triage.recommendation.priority;
  const approvalOverride = approvalRecommendation === "High" ? "Medium" : "High";
  approvalStore.approveIncidentPriority(approvalFirst.incident.id, approvalOverride, "Review Admin");
  assert.equal(approvalStore.getUnacknowledgedResidentReports().length, 0);
  assert.equal(approvalStore.getReport(approvalFirst.report.id).adminReview.acknowledgedBy, "Review Admin");
  assert.equal(approvalStore.getReport(approvalSecond.report.id).adminReview.acknowledged, true);
  assert.equal(approvalStore.getReport(approvalFirst.report.id).evidence[0].imageRef, "PHOTO-A");
  assert.ok(approvalStore.getIncident(approvalFirst.incident.id).linkedReportIds.includes(approvalSecond.report.id));
  assert.ok(approvalDomains.includes("reports"));
  assert.ok(approvalDomains.includes("incidents"));
  ...
  const staleReviewStore = makeStore({
    secureReportReports: [{
      id: "SR-STALE",
      source: "resident_report",
      description: "Report reviewed before acknowledgement lifecycle fix.",
      outageType: "Power outage",
      timestamp: "2026-09-21T10:00:00.000Z",
      incidentId: "INC-STALE",
      latitude: -25.75,
      longitude: 28.19,
      adminReview: { acknowledged: false, acknowledgedAt: null, acknowledgedBy: null },
    }],
    secureReportIncidents: [{
      ...
      longitude: 28.19,
      outageType: "Power outage",
      linkedReportIds: ["SR-STALE"],
      linkedSignals: [],
      priority: "Medium",
      triage: {
        state: "approved",
        recommendation: { priority: "Medium", reasons: [], generatedAt: "2026-09-21T10:00:00.000Z" },
        adminPriority: "Medium",
        reviewedBy: "Legacy Admin",
        reviewedAt: "2026-09-21T11:00:00.000Z",
        deferredAt: null,
      },
      timeline: [
        { type: "REPORT_RECEIVED", timestamp: "2026-09-21T10:00:00.000Z", message: "Report received", source: "resident_report", metadata: { reportId: "SR-STALE" } },
        { type: "ADMIN_PRIORITY_APPROVED", timestamp: "2026-09-21T11:00:00.000Z", message: "Priority approved", source: "admin", metadata: {} },
      ],
    }],
  });
  assert.equal(staleReviewStore.getReport("SR-STALE").adminReview.acknowledged, true);
  assert.equal(staleReviewStore.getReport("SR-STALE").adminReview.acknowledgedBy, "Legacy Admin");
  assert.equal(staleReviewStore.getUnacknowledgedResidentReports().length, 0);
