(function () {
  ...
  function migrateLegacyData() {
    ...
    // Repair pre-fix records whose linked incident was reviewed while the report
    // acknowledgement flag was left pending. Reports linked after the latest
    // review remain pending and can still notify Admin once.
    reports.forEach(function (report) {
      if (report.source !== "resident_report" || report.adminReview.acknowledged === true) return;
      const incident = incidents.find(function (item) {
        return item.id === report.incidentId && item.linkedReportIds.includes(report.id);
      });
      if (!incident || !incident.triage || !["approved", "deferred"].includes(incident.triage.state)) return;
      const reviewTypes = ["ADMIN_PRIORITY_APPROVED", "ADMIN_PRIORITY_OVERRIDDEN", "ADMIN_REVIEW_DEFERRED"];
      let reviewEventIndex = -1;
      (incident.timeline || []).forEach(function (event, index) {
        if (reviewTypes.includes(event.type)) reviewEventIndex = index;
      });
      const reviewEvent = reviewEventIndex >= 0 ? incident.timeline[reviewEventIndex] : null;
      const reviewTimestamp = incident.triage.reviewedAt || incident.triage.deferredAt || (reviewEvent && reviewEvent.timestamp);
      if (!reviewTimestamp) return;
      let reportEventIndex = -1;
      (incident.timeline || []).forEach(function (event, index) {
        if (event.metadata && event.metadata.reportId === report.id) reportEventIndex = index;
      });
      const reportTime = new Date(report.timestamp).getTime();
      const reviewTime = new Date(reviewTimestamp).getTime();
      const existedBeforeReview = reportTime < reviewTime ||
        (reportTime === reviewTime && reportEventIndex >= 0 && reviewEventIndex >= 0 && reportEventIndex < reviewEventIndex);
      if (!existedBeforeReview) return;
      report.adminReview = {
        acknowledged: true,
        acknowledgedAt: reviewTimestamp,
        acknowledgedBy: incident.triage.reviewedBy || "Admin",
      };
    });
  ...
  function acknowledgeLinkedResidentReports(reports, incident, adminName, acknowledgedAt) {
    const linkedIds = new Set(incident && incident.linkedReportIds || []);
    let changed = 0;
    reports.forEach(function (report) {
      if (!linkedIds.has(report.id) || report.source !== "resident_report" || report.adminReview.acknowledged === true) return;
      report.adminReview = {
        acknowledged: true,
  ...
  function approveIncidentPriority(incidentId, selectedPriority, reviewedBy) {
    ...
    if (acknowledgeLinkedResidentReports(reports, incident, incident.triage.reviewedBy, reviewedAt)) {
      saveReports(reports);
    }
    saveIncidents(incidents);
    return incident;
  ...
  function deferIncidentReview(incidentId, reviewedBy) {
    ...
    if (acknowledgeLinkedResidentReports(reports, incident, incident.triage.reviewedBy, deferredAt)) {
      saveReports(reports);
    }
    saveIncidents(incidents);
    return incident;
